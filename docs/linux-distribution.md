...
Distributions are built from the Linux sources and each distribution makes its own selection of versions and combination of packages.
...
Usually companies pick long term releases and this choice should usually work for Domino.
...
Before CentOS Stream many distributions have been built based on CentOS.
...
Both are valid choices. Amazon Linux is of special interest when running on AWS.
...
It has its own package manager **dpkg** and package format "Debian packages".
...
Another important detail is that Ubuntu is the only distribution with fully integrated ZFS support out of the box.
...
Domino 14 is built on RedHat Enterprise Linux 9.1, which runs on glibc 2.34.
